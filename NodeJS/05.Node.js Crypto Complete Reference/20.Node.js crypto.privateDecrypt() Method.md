O método crypto.privateDecrypt() é usado para descriptografar o conteúdo do buffer com privateKey.buffer que foi previamente criptografado usando a chave pública correspondente, ou seja, crypto.publicEncrypt().
Sintaxe:
```
...
privateKey: Pode conter dados do tipo Object, string, Buffer ou KeyObject.

oaepHash É a função de hash da cadeia de caracteres digitada que é usada para preenchimento de "OAEP". E o valor padrão é 'sha1'.
...
O exemplo abaixo ilustra o uso do método crypto.privateDecrypt() no Node.js:
### Exemplo 1:
```node
...
Referência: https://nodejs.org/api/crypto.html#crypto_crypto_privatedecrypt_privatekey_buffer
