O método crypto.randomFillSync() é uma interface de programação de aplicativo embutida do módulo crypto que é usada para retornar o objeto passado como argumento do buffer.
Sintaxe:
```
...
O exemplo abaixo ilustra o uso do método crypto.randomFillSync() no Node.js:
### Exemplo 1:
```node
...
Aqui, qualquer instância de TypedArray ou DataView é passada como buffer.
Referência: https://nodejs.org/api/crypto.html#crypto_crypto_randomfillsync_buffer_offset_size
